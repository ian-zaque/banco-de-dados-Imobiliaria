...
CREATE TABLE IF NOT EXISTS `bdImobiliaria`.`endereco` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `ID_UNIQUE` (`ID` ASC) VISIBLE,
  UNIQUE INDEX `id_bairro_UNIQUE` (`id_bairro` ASC) VISIBLE,
  UNIQUE INDEX `numero_casa_UNIQUE` (`numero_casa` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bdImobiliaria`.`imovel` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `ID_UNIQUE` (`ID` ASC) VISIBLE,
  UNIQUE INDEX `endereco_UNIQUE` (`id_endereco` ASC) VISIBLE,
  UNIQUE INDEX `id_fotos_UNIQUE` (`id_fotos` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bdImobiliaria`.`relacao_cliente_usuario_imovel` (
  ...
  UNIQUE INDEX `ID_UNIQUE` (`ID` ASC) VISIBLE,
  INDEX `id_imovel_idx` (`id_imovel` ASC) VISIBLE,
  INDEX `id_cliente_usuario_idx` (`id_cliente_usuario` ASC) VISIBLE,
  CONSTRAINT `id_imovel`
    FOREIGN KEY (`id_imovel`)
    REFERENCES `bdImobiliaria`.`imovel` (`ID`)
    ON DELETE NO ACTION
...
CREATE TABLE IF NOT EXISTS `bdImobiliaria`.`transacao` (
  ...
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `ID_UNIQUE` (`ID` ASC) VISIBLE,
  INDEX `id_pagamento_idx` (`id_pagamento` ASC) VISIBLE,
  INDEX `id_func_responsavel_idx` (`id_func_responsavel` ASC) VISIBLE,
  INDEX `id_rel_usuario_imovel_idx` (`id_rel_clienteUsuario_imovel` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `bdImobiliaria`.`tel_email` (
  ...
  UNIQUE INDEX `telefone_contato_UNIQUE` (`telefone_contato` ASC) VISIBLE,
  UNIQUE INDEX `telefone_celular_UNIQUE` (`telefone_celular` ASC) VISIBLE,
  UNIQUE INDEX `email_UNIQUE` (`email` ASC) VISIBLE,
